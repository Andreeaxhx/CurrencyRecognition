...
def sift(image, test_dir):
    ...
    sift = cv2.xfeatures2d.BRIEF_create()
    key_points, descriptors = sift.detectAndCompute(img, None) #we can add masks over the image instead of none
    ...
    for root, subdirs, files in os.walk(test_dir):
        for dir in subdirs:
            ...
            sift2 = cv2.xfeatures2d.BRIEF_create()
            img2 = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
            key_points2, descriptors2 = sift2.detectAndCompute(img2, None)  # we can add masks over the image instead of none
    ...
    print("[SIFT] the winner is: ", inverted_no_of_matches[maxim_no_of_matches])
