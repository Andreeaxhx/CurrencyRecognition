...
def orb(image, test_dir):
    ...
    orb = cv2.ORB_create()
    key_points, descriptors = orb.detectAndCompute(img, None) #we can add masks over the image instead of none
    ...
    for root, subdirs, files in os.walk(test_dir):
        for dir in subdirs:
            ...
            key_points2, descriptors2 = orb2.detectAndCompute(img2, None)  # we can add masks over the image instead of none
    ...
    print("[ORB] the winner is: ", inverted_no_of_matches[maxim_no_of_matches])
